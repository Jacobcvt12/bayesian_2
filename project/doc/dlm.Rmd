...
- Online forecasting
    - Not really relevant to my problem but an interesting 
      finding.
- While I did not consider this, DLMs can also be used in combination with
  regression.
...
y <- cases$cases
dates <- cases$date
data.model <- data.frame(dlm.model(y, order=1)) %>%
    mutate(x=dates)
...
- Weather likely plays a role in this data
    - Next step would likely include collecting weather data to use in a 
      DLM regression.
- Forecasting more than one step ahead may be useful, but credible intervals
  get very wide.
